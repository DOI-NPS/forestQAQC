...
library(htmlwidgets)
#park = "WEFA"; year = as.numeric(2022); panel = 4
# params <- data.frame(yearpv = 2022, year_curr = 2026, parkcode = "WEFA", panel = 4, print = FALSE)
# importData()
...
PI <- joinQuadSpecies(park = park, from = year, to = year, eventType = 'all', panel = panel) |> 
  filter(ScientificName %in% "Toxicodendron radicans")

PI_count <- PI |> 
            mutate(PI_quads = num_quads*(quad_pct_freq/100)) |> 
            filter(PI_quads > 0) |>  # Not needed b/c there has to be 1 quad present for toxrad
            mutate(Concerns = paste0("PI in ", PI_quads, " quads")) |> 
            select(Plot_Name, Concerns) 

PI_max <- PI |> rowwise() |> mutate(PI_max = max(c_across(starts_with("Pct_Cov")))) |> data.frame() |> 
  mutate(Concerns = paste0("Max PI = ", PI_max, "%")) |> 
  select(Plot_Name, Concerns)

PI_mean <- PI |> mutate(PI_avg = round(quad_avg_cov, 1)) |> 
                 mutate(Concerns = paste0("Avg. PI =  ", PI_avg, "%")) |> 
                 select(Plot_Name, Concerns) 
...
concerns <- rbind(stand_conc, shrubs, soil_sch1, cwd_conc, PI_count, PI_max, PI_mean) |> 
