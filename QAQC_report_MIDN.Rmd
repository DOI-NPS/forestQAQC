...
library(knitr)
library(kableExtra)
library(xml2) #for modifying HTML under kable hood

server_add <- read.csv('../server.csv')[1,]
importData(instance = "server", server = server_add)

#forestMIDN::importCSV(path = ./data/", zip_name = "MIDN_Forest_20220316.zip") # zip after new views validated
```
...
# Error check:
qaqc <- forestMIDN::joinLocEvent(QAQC = TRUE, locType = loc_type, eventType = 'all') %>%
...
row_changes_tr1 <- sapply(seq_along(tree_hts_wide), function(x){
  check = ifelse(tree_hts_wide$CrownClassLabel[x] != tree_hts_wide$CrownClassLabel[x + 1], x, NA)
  }) %>% data.frame() %>% na.omit(.)

row_changes_tr <- ifelse(nrow(row_changes_tr1) == 0, 0, row_changes_tr1[,1])
...
tree_tab <- kable(tree_wide2 %>% select(-fol_diff, -dec_diff, -HWA_diff, -BBD_diff), 
                  ...
                  col.names = c("Tree", "Species", "Crew", "QAQC", "Diff", "Crew", "QAQC",
                                ...
                                "Crew", "QAQC", "Crew", "QAQC", "Crew", "QAQC")) %>% 
            kable_styling(fixed_thead = TRUE, bootstrap_options = 'condensed', 
                          full_width = TRUE, position = 'left', font_size = 12.5) %>% 
            ...
            column_spec(10:11, background = 
                          case_when(as.vector(abs(tree_wide2$CrownClassCode_C - 
                                                    tree_wide2$CrownClassCode_Q) > 1) ~ diff_bad,
                                    as.vector(abs(tree_wide2$CrownClassCode_C - 
                                                    tree_wide2$CrownClassCode_Q) == 1) ~ diff_ok,
                                    TRUE ~ "#ffffff")) %>% 
...
shrub_tab_ta <- if(nrow(shrub_spp) > 0){ # this is prelim df to shrub_spp_wide
                   kable(shrub_spp_wide2 %>% select(ScientificName, crew, qaqc), digits = 1,
                   ...
                   column_spec(1:3, border_right = TRUE, border_left = T, 
                               background = case_when(shrub_spp_wide2$missed_c == 1 ~ "#ACC1DE",
                                                      shrub_spp_wide2$missed_q == 1 ~ "#DEACAC",
                                                      TRUE ~ "#ffffff")) %>% 
                   ...
                   row_spec(nrow(shrub_spp_wide2), bold = T, extra_css = "border-bottom: 1px solid #000000;",
                            background = 
                              ifelse(!is.na(shrub_spp_wide2[nrow(shrub_spp_wide2), 3]) &
                                       shrub_spp_wide2[nrow(shrub_spp_wide2), 3] < 1, 
                                     diff_bad, 
                                     "#ffffff"))
...
seeds_tab_ta <- if(nrow(sd_spp_to_include_15) + nrow(sd_spp_to_include_30) > 0){ 
                   ...
                   column_spec(1, border_right = TRUE, border_left = T, 
                               background = case_when(seeds_spp_wide2$missed_c_15 == 1 ~ "#ACC1DE",
                                                      seeds_spp_wide2$missed_q_15 == 1 ~ "#DEACAC",
                                                      TRUE ~ "#ffffff")) %>% 
                   column_spec(4:5, border_right = TRUE, border_left = T, 
                               background = case_when(seeds_spp_wide2$missed_c_30 == 1 ~ "#ACC1DE",
                                                      seeds_spp_wide2$missed_q_30 == 1 ~ "#DEACAC",
                                                      TRUE ~ "#ffffff")) %>% 
                   ...
                   column_spec(4:5, background = 
                              ifelse(!is.na(seeds_spp_wide2[nrow(seeds_spp_wide2), 5]) &
                                       seeds_spp_wide2[nrow(seeds_spp_wide2), 5] < 1,  diff_bad, "#ffffff"),
                               extra_css = "border-bottom: 1px solid #000000;")
...
chr_tab <- kable(quad_chr_comb %>% select(Character:CC_Q), format = 'html', align = 'c', 
           ...
           add_header_above(c("", "A2" = 2, "A5" = 2, "A8" = 2, "AA" = 2, 
                            ...
                            extra_css = "border-bottom: 1px solid #000000; border-right: 3px solid #ffffff;",
                            line = FALSE) %>% 
           column_spec(2:3, background = check_dif_col(quad_chr_comb, "A2_dif")) %>% 
           column_spec(4:5, background = check_dif_col(quad_chr_comb, "A5_dif")) %>% 
           column_spec(6:7, background = check_dif_col(quad_chr_comb, "A8_dif")) %>% 
           column_spec(8:9, background = check_dif_col(quad_chr_comb, "AA_dif")) %>% 

           column_spec(10:11, background = check_dif_col(quad_chr_comb, "B2_dif")) %>% 
           column_spec(12:13, background = check_dif_col(quad_chr_comb, "B5_dif")) %>%
           column_spec(14:15, background = check_dif_col(quad_chr_comb, "B8_dif")) %>% 
           column_spec(16:17, background = check_dif_col(quad_chr_comb, "BB_dif")) %>%

           column_spec(18:19, background = check_dif_col(quad_chr_comb, "C2_dif")) %>% 
           column_spec(20:21, background = check_dif_col(quad_chr_comb, "C5_dif")) %>%
           column_spec(22:23, background = check_dif_col(quad_chr_comb, "C8_dif")) %>% 
           column_spec(24:25, background = check_dif_col(quad_chr_comb, "CC_dif")) %>%
...
#+++ NEW 20220321 +++
quad_tab_ta <- if(nrow(quad_spp) > 0){ # this is prelim df to quad_spp_wide, only includes spp >=1% cover
                   kable(quad_spp_wide2 %>% select(ScientificName, crew, qaqc),
                         format = 'html', align = 'c', col.names = c("Species", "Crew", "QAQC")) %>% 
                   kable_styling(fixed_thead = TRUE, bootstrap_options = 'condensed',
                                 full_width = FALSE, position = 'left', font_size = 14) %>% 
                   column_spec(1:3, border_right = TRUE, border_left = T, 
                               background = case_when(quad_spp_wide2$missed_c == 1 ~ "#ACC1DE",
                                                      quad_spp_wide2$missed_q == 1 ~ "#DEACAC",
                                                      TRUE ~ "#ffffff")) %>% 
...
plotspp_tab_ta <- if(nrow(plot_spp) > 0){ # this is prelim df to plot_spp_wide
                   ...
                   kable_styling(fixed_thead = TRUE, bootstrap_options = 'condensed',
                                 full_width = FALSE, position = 'left', font_size = 14) %>% 
                   column_spec(1:3, border_right = TRUE, border_left = T, 
                               background = case_when(plot_spp_wide2$missed_c == 1 ~ "#ACC1DE",
                                                      plot_spp_wide2$missed_q == 1 ~ "#DEACAC",
                                                      TRUE ~ "#ffffff")) %>% 
...
```{r, tree_table, echo = FALSE, error = TRUE}
#if(nrow(tree_wide2) > 1){
tree_tab
...
```{r diff_plot_printed, error = TRUE}
#if(nrow(tree_wide2) > 1){
diff_plot
#} else {
#  cat("Trees not sampled")
...
seed_TA_sm <- c("Taxonomic Accuracy", "Small Seedling Similarity", "Sorensen similarity >= 0.9 and < 1", 
                "Sorensen similarity < 0.9 for seedlings in the 15 -- 30 cm size class and having at least 3 stems present across the quadrats")
...
data_checks <- rbind(data_checks, crown_closure, water_on_plot, DBI, microtopography, earthworms, vasc_strata,
                     ...
                     sap_dbh, micro_sap, micro_sap_dbh, micro_shrubs, micro_shrubs_spp, micro_regen,
                     ...
                     shrub_TA, seed_TA_sm, seed_TA_lg, quad_TA, plot_TA
                     )
